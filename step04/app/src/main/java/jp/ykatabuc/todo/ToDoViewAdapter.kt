...
class ToDoViewAdapter(
    ...
    override fun onBindViewHolder(holder: ToDoViewHolder, position: Int) {
        ...
        // リスナー設定
        holder.itemView.setOnClickListener { holder.checkCb.isChecked = !holder.checkCb.isChecked }

        holder.checkCb.setOnCheckedChangeListener { _, _ -> onClick(item) }
    }

    fun updateItems(newItems: List<ToDo>) {
        val diffResult =
            DiffUtil.calculateDiff(ToDoDiffUtilCallback(todoList, newItems), false)
...
class ToDoViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    val checkCb: CheckBox = view.cell_todo_cb
}